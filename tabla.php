...
/**
 * Presenta registros de una tabla b�sica
 * Referencias:
...
function registro(&$d, $titulo, $k)
{
    ...
    if (is_array($titulo)) {
        ...
        $p = " (";
        foreach ($titulo as $c) {
            // getLink no soporta llaves foraneas m�ltiples
 ...
 * @param string  $idpapa  Id. de pap�
 * @param boolean $arbol   Es �rbol?
...
if (!in_array($tabla, $u)) {
    die("La tabla '$tabla' no es b�sica");
}
...
$vd = get_object_vars($d);
if (in_array('id_papa', array_keys($vd))) { /** jerarqu�a */
