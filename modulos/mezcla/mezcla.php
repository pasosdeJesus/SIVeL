 ...
 * @param array  $estbd      Estructura de base de datos ($tabla => ($campo1 => $tipo1, ... $campon => $tipon))
...
function mezclaen($id1, $id2, $elim2, &$obs, &$rvic, &$fecha, &$rdep) {
    ...
    while($do2->fetch()) {
        ...
        if (!$do1->fetch()) {
            ...
            $obs .= " Asociado caso_presponsable(id_presponsable:{$do2->id_presponsable})";
    ...
    while($do2->fetch()) {
        ...
        if (!$do1->fetch()) {
            ...
            $obs .= " Asociado caso_categoria_presponsable(id_presponsable:{$do2->id_presponsable})";
        }
    }
