...
/**
 * Consulta para la p�gina web.
 * Una versi�n inicial que sirvi� de referencia fue desarrollada por
...
/**
 * Responde a bot�n para hacer consulta.
 *
 * @category SIVeL
 * @package  SIVeL
 * @author   Vladimir T�mara <vtamara@pasosdeJesus.org>
...
class AccionConsultaWeb extends HTML_QuickForm_Action
{
    ...
    function perform(&$page, $actionName)
    {
        ...
        // B�squeda por v�ctima no incluye combatientes para evitar sobreconteos
...
class ConsultaWeb extends HTML_QuickForm_Page
{
    ...
    function buildForm()
    {
        ...
        if ($this->opciones != array()) {
            $cod =& $this->addElement('text', 'id_casos', 'C�digo(s): ');
            $cod->setSize(80);
        }
        ...
        $sel =& $this->addElement(
            'text', 'nomvic',
            'Nombre o apellido de la v�ctima'
        );
        $sel->setSize(80);
        ...
        if (in_array(42, $this->opciones)) { // Podr�a ver rep. gen?
            $sel =& $this->createElement(
                'checkbox',
        ...
        $sel =& $this->createElement(
            'checkbox',
            'retroalimentacion', 'Retroalimentaci�n', 'Retroalimentaci�n'
        );
        $sel->setValue(false);
        $opch[] =& $sel;
        ...
        $sel =& $this->createElement(
            'checkbox',
            'm_tex', 'Conversi�n a TeX', 'Conversi�n a TeX'
        );
        ...
        $this->addGroup(
            $opch, null, 'Detalles de la presentaci�n',
            '&nbsp;', false
        );
