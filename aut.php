...
/**
 * Funciones para autenticaci�n de usuarios con Auth de Pear.
 ...
 * Debe incluirse despu�s de incluir misc.php.
 *
 * PHP version 5
 *
 * @category  SIVeL
...
function sacaOpciones($usuario, &$db, &$op, &$rol)
{
    ...
    if (PEAR::isError($result)) {
        die("No se encontr� el rol del usuario");
    }
    $row = array();
...
/**
 * Retorna nombre de la sesi�n basado en URL.
 * Podr�a considerarse una forma de almacenar sesiones diferente --drupal
 * usa base de datos.
 *
 * @return string Nombre de la sesi�n
 */
function nomSesion()
{
    ...
    // al menos en php 5.2.5 de OpenBSD 4.2 el nombre de la sesi�n debe
    // ser alfanum�rico (documentado) y comenzar con letra (no documentado).
    // As� mismo varias instalaciones en el mismo HOST corriendo simultaneamente
    // confundir�an el nombre de sesi�n.
    $snru = preg_replace(
        '/[^a-z0-9]/i', '',
...
/**
 * De requerirlo inicia sesi�n y autentica un usuario listado como usuario
 ...
 * @param integer $opcion   C�digo de la opci�n del menu que el usuario desea
 *
...
function autenticaUsuario($dsn,  $accno, &$usuario, $opcion)
{
    ...
    if ($a->checkAuth()) {
        ...
        $texp = 60*60*4; // 4 horas de sesi�n
        ...
        if (!isset($_SESSION['opciones'])
            ...
            if (count($op) == 0) {
                echo "No tiene opciones este usuario, " .
                    "�seguro la base est� bien inicializada?";
            }
...
/**
 * Cierra sesi�n iniciada con autenticaUsuario.
 *
 * @param string $dsn URL de base de datos
 *
 * @return nada
...
/**
 * Localiza directorio del archivo de configuraci�n seg�n URL
 *
 * @return nada
 */
function localizaConf()
{
    ...
    if (!$existe) {
        die("No existe configuraci�n '$dirsitio'");
    }
