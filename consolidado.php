...
class AccionConsolidado extends HTML_QuickForm_Action
{
     ...
     * @param unknown $ncol   N�mero de columnas
     *
     * @return void
     */
     ...
     * Realiza consulta
     *
     * @param unknown &$page      P�gina
     * @param unknown $actionName nombre de la acci�n
     *
     * @return void
     * @access public
     */
    function perform(&$page, $actionName)
    {
        ...
        if ($ncol <= 0) {
            die('Falta informaci�n para reporte consolidado en tabla categoria');
        }
        $d->orderBy('id');
        $d->find();
        while ($d->fetch()) {
        ...
        while ($result->fetchInto($row)) {
            ...
            if ($fhastallena || $fdesdellena) {
                /* Se hace as� porque puede haber casos que no
        ...
        if ($pMuestra == "tabla") {
            ...
            echo "<th>Fecha</th><th>Ubicaci�n</th><th>V�ctimas</th>";
        } elseif ($pMuestra == "csv") {
            header("Content-type: text/csv");
            echo '"Fecha", "Ubicaci�n", "V�ctimas", ""';
        } elseif ($pMuestra == 'latex') {
            //header("Content-type: application/x-latex");
            echo "<pre>";
            echo '\\textbf{Fecha} & \\textbf{Ubicacion} & \\textbf{V�ctimas} ';
        }
        ...
        for ($v = 0;$v < $tv; $v++) {
            ...
            if ($u->find() == 0) {
                die("<br/><font color='red'>Caso sin ubicacion: " .
                    "$idcaso -- estad�stica incompleta!!!</font>"
                );
            }
            $u->fetch();
                ...
                if (isset($dv[$idcaso][$idvic][$idcat])) {
                    ...
                    if ($dv[$idcaso][$idvic][$idcat] == 1) {
                        ...
                        die("Sobreconteo por caso " . $idcaso .
                            " que saldr�a m�s de una vez en la columna " .
                            $idcat
                        );
                    }
            ...
            if ($acto->find()==0) {
                $acto = null;
                echo_esc(
                    "V�ctima sin presunto responsable '$idvic', " 
                    . "'$nom' en caso '$idcaso' -- estad�stica incompleta!!!"
                );
            } else {
                $apr = array();
                $pr_sep = $presp = "";
...
class PagConsolidado extends HTML_QuickForm_Page
{
    ...
    function buildForm()
    {
        ...
        $ae[] =&  $this->createElement(
            'radio', 'muestra', 'csv',
            'Formato CSV (hoja de c�lculo)', 'csv'
        );
        $ae[] =&  $this->createElement(
            'radio', 'muestra', 'latex',
            'LaTeX', 'latex'
        );
        $this->addGroup($ae, null, 'Forma de presentaci�n', '&nbsp;', false);
        $t->setChecked(true);
